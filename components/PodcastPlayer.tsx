...
/* Toggles the mute/unmute state of the audio element */
  const toggleMute = () => {
    if (audioRef.current) {
...
/* Updates the current time of the audio element */
  useEffect(() => {
...
/* Plays the audio when the audio changes */
  useEffect(() => {
    const audioElement = audioRef.current;
    if (audio?.audioUrl) {
      if (audioElement) {
